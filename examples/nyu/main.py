...
def parse_args(parser):
    ...
    parser.add_argument('--single_task', type=str, default=None, help='train only one task')
    parser.add_argument('--stl_metrics_path', type=str, default=None, help='path to STL metrics')
...
def main(params):
    ...
    if params.single_task is not None:
        task_dict = {params.single_task: task_dict[params.single_task]}
    ...
    if params.mode == 'train':
        if params.weighting == "stl":
            with open(params.stl_metrics_path, "r") as f:
                stl_metrics = [json.loads(line) for line in f]
            # 假设顺序和 task_dict 一致
            weights = {m["task"]: 1.0 / m["best_metric"] for m in stl_metrics}
        NYUmodel.train(nyuv2_train_loader, nyuv2_test_loader, params.epochs)
        result = {"task": params.single_task, "best_metric": best_metric_value}
        with open("nyu_stl_results.json", "a") as f:
